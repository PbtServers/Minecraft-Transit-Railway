    ...
    public static void render(GraphicsHolder graphicsHolder) {
        ...
        if (vehicle != null && (minecraftClient.getCurrentScreenMapped() == null || minecraftClient.getCurrentScreenMapped().getTitle().data.toString().contains("chat_screen.title")) && VehicleRidingMovement.getValidHoldingKey(vehicle.vehicleExtraData.getDepotId()) != null) {
            ...
            for (int i = 0; i < 180; i += SPEEDOMETER_CIRCLE_INTERVAL) {
                ...
                guiDrawing.drawRectangle(-radius, -(float) SPEEDOMETER_CIRCLE_EDGE_LENGTH / 2, radius, (float) SPEEDOMETER_CIRCLE_EDGE_LENGTH / 2, 0xFF666666);
                guiDrawing.drawRectangle(-radius + 1, -(float) SPEEDOMETER_CIRCLE_EDGE_LENGTH / 2, radius - 1, (float) SPEEDOMETER_CIRCLE_EDGE_LENGTH / 2, 0xFF111111);
            ...
            for (float i = 0; i < ATS_SLICES; i++) {
                guiDrawing.beginDrawingRectangle();
                guiDrawing.drawRectangle(-ATS_RADIUS_1, -(float) ATS_CIRCLE_EDGE_HALF_LENGTH_1, ATS_RADIUS_1, (float) ATS_CIRCLE_EDGE_HALF_LENGTH_1, 0xFF1A1A1A);
                guiDrawing.finishDrawingRectangle();
            ...
            for (int i = 0; i <= SPEEDOMETER_MAX_DISPLAY; i += SPEEDOMETER_TICK_INTERVAL) {
                guiDrawing.beginDrawingRectangle();
                float tickLength = (i % 100 == 0 ? 10 : i % 50 == 0 ? 7 : 4);
                guiDrawing.drawRectangle(
                        -radius + 2, -0.5F,
                        -radius + 2 + tickLength, 0.5F,
                        vehicle.getSpeedLimitKilometersPerHour() > 0 && i == Math.min(vehicle.getSpeedLimitKilometersPerHour(), SPEEDOMETER_MAX_DISPLAY) ? 0xFF00FF00 : IGui.ARGB_WHITE
                );
                guiDrawing.finishDrawingRectangle();
                graphicsHolder.rotateZDegrees((float) SPEEDOMETER_TICK_INTERVAL * SPEEDOMETER_SPAN / SPEEDOMETER_MAX_DISPLAY);
            }
            graphicsHolder.pop();
            ...
            for (int i = 0; i <= SPEEDOMETER_MAX_DISPLAY; i += 100) {
                graphicsHolder.push();
                graphicsHolder.translate(-radius + 16 + IGui.LINE_HEIGHT / 2F, 0, 0);
                graphicsHolder.rotateZDegrees(-SPEEDOMETER_START_ANGLE - (float) i * SPEEDOMETER_SPAN / SPEEDOMETER_MAX_DISPLAY);
                graphicsHolder.scale(0.45F, 0.45F, 1);
                drawCenteredText(graphicsHolder, String.valueOf(i), IGui.ARGB_WHITE);
                graphicsHolder.pop();
                if (i < SPEEDOMETER_MAX_DISPLAY) {
                    graphicsHolder.rotateZDegrees(100F * SPEEDOMETER_SPAN / SPEEDOMETER_MAX_DISPLAY);
                }
            }
            graphicsHolder.pop();
            ...
            guiDrawing.beginDrawingRectangle();
            guiDrawing.drawRectangle(-radius + 5, -0.5F, -5, 0.5F, 0xFFFF0000);
            guiDrawing.finishDrawingRectangle();
            graphicsHolder.pop();
