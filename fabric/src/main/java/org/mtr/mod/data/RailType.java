...
public enum RailType implements IGui {
    ...
    HIGHSPEED_RUNWAY(600, MapColor.getPalePurpleMapped(), false, true, false, Rail.Shape.QUADRATIC),
    ...
    public static int getRailColor(Rail rail) {
        if (rail.isPlatform()) {
            ...
            // Lógica para pistas de aterrizaje
            final int speed = (int) Math.max(rail.getSpeedLimitKilometersPerHour(false), rail.getSpeedLimitKilometersPerHour(true));
            return speed > RUNWAY.speedLimit ? HIGHSPEED_RUNWAY.color : RUNWAY.color;
        } else {
            final int speed = (int) Math.max(rail.getSpeedLimitKilometersPerHour(false), rail.getSpeedLimitKilometersPerHour(true));
            for (final RailType railType : values()) {
                // Filtramos para no devolver colores de tipos especiales (plataformas, sidings o pistas) por error
                if (!railType.isSavedRail && railType != PLATFORM && railType != SIDING && railType != TURN_BACK && railType != RUNWAY && railType != HIGHSPEED_RUNWAY) {
                    if (railType.speedLimit == speed) {
